...
> Talvez você precise gerar um token para fazer o push com sucesso. Para isso, vá na página do Github e acesse as configurações (settings) da sua conta (você pode acessar clicando na sua imagem de usuário no canto superior direito). Depois escolha a opção "Developer settings", e por fim, a opção "Personal access tokens". Clique no botão "Generate New Token" para gerar um novo token. Dica: gere um token com permissão de repo marcando a caixa "repo" em "Select scopes". Essa opção permitirá você dar push nos seus commits (pelo menos, o Wendel só conseguiu dar push no commit dele depois de ajustar essa opção).
